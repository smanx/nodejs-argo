...
let UUID = process.env.UUID || uuidv4(); // 使用哪吒v1,在不同的平台运行需修改UUID,否则会覆盖
...
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'idx-523-demo.smanx.dpdns.org';          // 固定隧道域名,留空即启用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiNGE5ZjYxOWI2NDZiZjk2NzkyY2JlMjMwNWNmZTAxMTUiLCJ0IjoiNjVlYTQwODUtOTc0Yi00MTNlLTk1MTItNjMyZDlmNzAzMTUwIiwicyI6Ik5UVmtNek13TnpZdE1qSTRaUzAwTlRSaUxXSTFNR0V0TmpnNU5XUTFNV1kwTm1VMiJ9';              // 固定隧道密钥json或token,留空即启用临时隧道,json获取地址：https://fscarmen.cloudflare.now.cc
const ARGO_PORT = process.env.ARGO_PORT || 8001;            // 固定隧道端口,使用token需在cloudflare后台设置和这里一致
const CFIP = process.env.CFIP || '104.16.0.0';         // 节点优选域名或优选ip  
const CFPORT = process.env.CFPORT || 443;       // 节点优选域名或优选ip对应的端口
const NAME = process.env.NAME || 'idx-523';                     // 节点名称
